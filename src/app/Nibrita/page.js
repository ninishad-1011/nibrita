  ...
  return (
    <div className="relative flex justify-center items-center min-h-screen bg-gradient-to-br from-pink-200 via-pink-300 to-pink-400 overflow-hidden px-2">
      ...
      <h2 className="text-[50px]">🎆</h2>
        <h1 className="text-3xl sm:text-4xl font-bold text-pink-600 text-shadow animate-glow mb-2"> Happy New Year </h1>
        
        <div className="text-3xl text-pink-600 font-bold text-shadow animate-neon mb-4">💖 2026 💖</div>
        <img
        ...
        />
        <h2 className="font-bold text-xl sm:text-2xl text-pink-600 mb-2">💖 Nazmun Nahar 💖</h2>
        <p className="mt-2 font-medium text-pink-700 text-shadow">May your life be full of love & happiness ✨</p>
        <div className="text-5xl mt-5 animate-beat text-pink-600 text-shadow">💗</div>
        <button
          onClick={showMessage}
      ...
      {popupVisible && (
        <div className="fixed inset-0 bg-black/60 flex justify-center items-center p-5 z-20">
          <div className="bg-white text-center p-6 sm:p-8 rounded-2xl max-w-sm w-full animate-[zoom_0.5s_ease]">
            <h2 className="text-2xl sm:text-3xl text-pink-600 text-shadow mb-4">💖 My Special Message 💖</h2>
            <p className="text-pink-500">
              This new year I just want to say…<br/>
              You mean everything to me 🥰<br/>
              Your smile lights up my world 🌸<br/><br/>
              💍 Will you stay with me and make this year unforgettable together?
            </p>
            <button
              onClick={closePopup}
      ...
      <style jsx>{`
        ...
        @keyframes glow {0%{text-shadow:0 0 10px #ff84c4,0 0 20px #ff9edc;}100%{text-shadow:0 0 20px #ff84c4,0 0 30px #ffb3e0,0 0 40px #ffccee;}}
        @keyframes neon {0%{text-shadow:0 0 5px #ff8fab,0 0 10px #ffb3c6;}100%{text-shadow:0 0 20px #ff8fab,0 0 30px #ffb3c6,0 0 40px #ffc9db;}}
        @keyframes beat {0%,100%{transform:scale(1);}50%{transform:scale(1.3);}}
        @keyframes buttonGlow {0%{box-shadow:0 0 20px rgba(255,100,150,0.8);}100%{box-shadow:0 0 40px rgba(255,100,200,1);}}
        ...
        @keyframes sparkle {0%{opacity:0.3;transform:scale(0.5) rotate(0deg);}100%{opacity:1;transform:scale(1.5) rotate(180deg);}}
        @keyframes firework {0%{transform:scale(0);opacity:1;}50%{transform:scale(3);opacity:1;}100%{transform:scale(5);opacity:0;}}
        @keyframes flyUp {0%{transform:translateY(0) rotate(0deg);opacity:1;}100%{transform:translateY(-120vh) rotate(360deg);opacity:0;}}
        .animate-glow{animation:glow 1.5s ease-in-out infinite alternate;}
        .animate-neon{animation:neon 2s infinite alternate;}
        .animate-beat{animation:beat 1s infinite;}
        .animate-buttonGlow{animation:buttonGlow 1.5s infinite alternate;}
        .animate-zoom{animation:zoom 0.5s ease;}
        .text-shadow{text-shadow:0 0 5px #ff84c4,0 0 10px #ff9edc,0 0 20px #ffb3e0,0 0 30px #ffccee;}
      `}</style>
    </div>
  );
}
